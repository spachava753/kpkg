...
func (l GithubReleaseTool) Versions() ([]string, error) {
	...
	releases, resp, err := client.Repositories.ListReleases(context.Background(), l.org, l.repo, nil)
	if err != nil {
		return nil, err
	}
	var r []*github.RepositoryRelease
	for resp != nil && resp.NextPage != resp.LastPage && uint(len(releases)) < l.max {
		r, resp, err = client.Repositories.ListReleases(context.Background(), l.org, l.repo, &github.ListOptions{
			Page:    resp.NextPage,
